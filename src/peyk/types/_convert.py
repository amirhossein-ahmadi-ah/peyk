...
def chat_type(value: Optional[str]) -> ChatType:
    """Provides the chat type operation for the peyk integration.
...
def user(raw: object) -> User:
    """Provides the user operation for the peyk integration.
...
Returns:
    Result produced by the operation."""
    return Chat(id=getattr(raw, 'id', getattr(raw, 'chat_id', '')), type=chat_type(getattr(raw, 'type', getattr(raw, 'chat_type', None))), title=getattr(raw, 'title', None), username=getattr(raw, 'username', None), raw=raw)

def content_type(raw: object) -> ContentType:
    """Provides the content type operation for the peyk integration.
...
def message_base(raw: object, *, bot: object, chat_override: Optional[Chat]=None) -> Message:
    """Provides the message base operation for the peyk integration.
...
Returns:
    ...
    raw_reply = getattr(raw, 'reply_to_message', None)
    reply_message = message_base(raw_reply, bot=bot) if raw_reply is not None else None
    return Message(message_id=getattr(raw, 'message_id', None), chat_id=normalized_chat.id if normalized_chat else None, chat_type=normalized_chat.type.value if normalized_chat else None, sender_id=from_user.id if from_user else None, text=getattr(raw, 'text', None) if getattr(raw, 'text', None) is not None else getattr(raw, 'caption', None), date=getattr(raw, 'date', getattr(raw, 'time', None)), is_edited=bool(getattr(raw, 'is_edited', False) or getattr(raw, 'edit_date', None) is not None), reply_to_message_id=getattr(raw, 'reply_to_message_id', None) or getattr(raw_reply, 'message_id', None), reply_to_message=reply_message, media=next((getattr(raw, name) for name in ('photo', 'video', 'audio', 'voice', 'document', 'sticker', 'animation', 'contact', 'location', 'poll', 'venue') if getattr(raw, name, None) is not None), None), new_chat_members=[u.id for u in getattr(raw, 'new_chat_members', [])] if getattr(raw, 'new_chat_members', None) is not None else None, left_chat_member=getattr(getattr(raw, 'left_chat_member', None), 'id', None), successful_payment=getattr(raw, 'successful_payment', None), raw=raw, from_user=from_user, chat=normalized_chat, content_type=content_type(raw), bot=bot)
