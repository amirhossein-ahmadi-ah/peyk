...
Returns:
    ...
    if files:
        fields: Dict[str, str] = {'user_id': str(user_id), 'name': name, 'sticker': self._json_field(sticker_payload)}
        return bool(await self._call('addStickerToSet', data=fields, files=files))
    payload: Dict[str, object] = {'user_id': user_id, 'name': name, 'sticker': sticker_payload}
