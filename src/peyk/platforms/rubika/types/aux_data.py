...
class AuxData:
    ...
    Confirmed from the docs' ``receiveUpdate``/``receiveInlineMessage``
    example bodies.
    """
    ...
    @classmethod
    def from_dict(cls, data: Optional[dict]) -> Optional['AuxData']:
...
Returns:
    Result produced by the Rubika operation."""
        "Executes the from_dict operation.\n        \n        Args:\n            data: Value of the declared parameter type.\n        \n        \n        Returns:\n            The operation result (``Optional['AuxData']``).\n        "
        if data is None:
            return None
        return cls(start_id=_to_str(data.get('start_id')), button_id=_to_str(data.get('button_id')))
